---
title: "Vercel Portability Strategy"
description: "Strategy for maintaining platform portability while leveraging Vercel features, ensuring the API can be deployed anywhere without vendor lock-in."
---
...
**Framework Defined Infrastructure (FDI)** - Build with open frameworks (Next.js, Elysia) without proprietary APIs. Code remains portable while Vercel automatically provisions infrastructure.
...
- 2024 Backend Improvements: Zero-config backend support (Elysia, Next.js), Fluid Compute, reduced cold starts
...
- **Elysia Application**: Standard Elysia with no Vercel-specific code
- **API Endpoints**: Standard Elysia/HTTP patterns (functional, Zod-first)
...
- **Error Handling**: Standard Elysia error handling (functional patterns)
- **Validation**: Zod-first validation for all boundaries (requests, responses, environment variables)
...
- **Security Headers**: Elysia security headers plugin (standard)
- **Request Size Limits**: Elysia body parser limits (standard)
- **CORS**: Elysia CORS configuration (standard)
- **Rate Limiting**: Elysia rate limiting plugins (works on any Elysia app)
...
1. **Lambda**: Wrap Elysia app in Lambda handler
...
- ✅ Use standard Elysia/HTTP patterns (functional, Zod-first)
