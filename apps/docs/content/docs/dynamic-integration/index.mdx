...
description: "Architecture for integrating with Dynamic SDK, including single integration point pattern and type-safe token operations."
---
...
All Dynamic SDK calls go through the Vencura backend API:

```
Web App → Vencura API → Dynamic SDK
```
...
- **All wallet operations** (create, balance, sign, send) go through Vencura API
...
- It doesn't bypass the Vencura API for wallet operations
...
- **React hooks** for interacting with Vencura API
- **Type-safe token operations** (`useMintToken`, `useBurnToken`)
- **Encoding utilities** (`encodeTokenMint`, `encodeTokenBurn`)
- **No direct blockchain calls** - all operations go through Vencura API
...
1. **Single integration point**: All Dynamic SDK calls go through Vencura API
