...
description: "Decision to use backend-first (Elysia) architecture with AI SDK Core for AI chatbot functionality, providing natural language interface, React/Next.js embeddability, and AI-friendly architecture."
---
...
- Leverage **AI-friendly** architecture (clean HTTP contracts, OpenAPI, typed SDKs)
...
- Difficult to integrate with existing Elysia-based wallet services
- Not suitable for long-running wallet operations
- Cannot easily migrate to other platforms

### Option B – Backend-First (Elysia) with AI SDK Core

Use Elysia backend with `@ai-sdk/core` for provider-agnostic AI integration.
...
- Seamless integration with existing Elysia-based wallet services
- Standard SSE streaming works everywhere
- No vendor lock-in
- Can orchestrate multiple AI providers
- Better for wallet security and compliance
- **AI-friendly**: Clean HTTP contracts, OpenAPI generation, typed SDKs (`@vencura/core`)
- **Zod-first**: Request/response validation with Zod schemas for type-safe AI tool integration
...
### Option C – Hybrid (Vercel Edge + Elysia)

Use Vercel Edge for AI inference, Elysia for wallet operations.
...
We will use **Option B: Backend-First (Elysia) with AI SDK Core**.
...
- **Integration**: Seamless integration with existing Elysia-based wallet services
...
- **AI-friendly**: Clean HTTP contracts, auto-generated OpenAPI specs, typed SDKs (`@vencura/core`) enable seamless AI tool integration
- **Zod-first**: Request/response validation with Zod schemas ensures type-safe AI tool integration
...
Uses Elysia's functional SSE response pattern:
...
- **Elysia Integration**: Our existing infrastructure is Elysia-based (functional, Zod-first)
...
The Elysia backend can be deployed to:
