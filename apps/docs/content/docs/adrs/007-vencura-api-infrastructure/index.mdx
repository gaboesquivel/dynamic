...
description: "Decision to deploy Elysia-based Vencura API on Google Cloud Run with Docker containers and Cloudflare CDN for serverless auto-scaling, WebSocket support, and global distribution."
---

## Context

We need to select a deployment platform for the Elysia-based Vencura API that:
...
- Elysia requires adaptation for Lambda
...
**Current Deployment**: We deploy the Elysia-based Vencura API on **Vercel** for all environments. This leverages Vercel's zero-configuration backend support, Fluid Compute, reduced cold starts, and excellent developer experience.

### TLDR: Comparison Table

| Feature | Vercel ✅ (Current) | Google Cloud Run (Future Option) | AWS Lambda | Railway/Render |
|---------|---------------------|----------------------------------|------------|----------------|
| **Zero-config backend** | ✅ Elysia support | ⚠️ Docker required | ⚠️ Adaptation needed | ✅ Simple |
| **Cold starts** | ✅ Reduced (2024) | ⚠️ Can occur | ⚠️ Significant | ✅ Minimal |
| **WebSocket support** | ⚠️ Limited | ✅ Excellent | ❌ Complex | ⚠️ Limited |
| **Global distribution** | ✅ 100+ edge locations | ⚠️ Cloudflare CDN | ✅ CloudFront | ⚠️ Limited |
| **Developer experience** | ✅ Excellent | ⚠️ More complex | ⚠️ Complex | ✅ Good |
| **CI/CD integration** | ✅ Git push = deploy | ⚠️ Cloud Build setup | ⚠️ Manual | ✅ Simple |
| **Cost (low traffic)** | ✅ Pay-per-use | ⚠️ Min instance cost | ✅ Pay-per-use | ⚠️ Fixed pricing |
| **Security features** | ✅ SOC 2, ISO 27001 | ✅ Enhanced (HSM, VPC) | ✅ Good | ⚠️ Standard |
| **Portability** | ✅ Portable code | ✅ Docker containers | ⚠️ Lambda-specific | ✅ Docker |

**Documented Future Option**: Google Cloud Run with Docker containers and Cloudflare is documented as a potential future option for production workloads requiring enhanced security and data governance. This option would use:

- **Google Cloud Run** with Docker containers for serverless auto-scaling
- **Cloudflare** in front for CDN, DDoS protection, and global distribution
- **Minimum instance count of 1** to eliminate cold starts and ensure consistent performance

**Main reasons for Vercel (current)**:

- Zero-configuration backend support for Elysia and Next.js
- Fluid Compute with Active CPU pricing (automatic scaling, pay-for-what-you-use)
- Significantly reduced cold starts for backend APIs
- Native support for long-running backend applications
- Unparalleled shipping and distribution capabilities
- Excellent developer experience with MCP integration

**Main reasons for Google Cloud (future option, if needed)**:

- Serverless containers provide auto-scaling with pay-per-use pricing
- Native Docker support simplifies deployment
- Excellent WebSocket support for real-time features
- Enhanced security features for production workloads (HSM-backed keys, private networking)
- Good integration with Google Cloud services (Cloud SQL, Pub/Sub, etc.)
...
**Current Deployment**: ⚠️ **All systems are currently deployed to Vercel** for all environments. We are NOT splitting the architecture now - everything stays on Vercel to leverage its unparalleled shipping and distribution capabilities.
...
- Thin Elysia-based adapters for user auth, dashboards, webhooks, notifications
- Public API facades
- Leverages Vercel's excellent DX, edge network, and integrations

**Key-Custody & Signing Core on Google Cloud (Only if needed):**

- Elysia-based "signer" service on Cloud Run in private VPC
