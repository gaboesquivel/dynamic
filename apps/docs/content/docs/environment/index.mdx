...
All apps enforce **Zod-validated** environment configuration objects using Zod schemas and `@vencura/lib` utilities. This ensures:
...
- Use `validateEnvOrThrow` from `@vencura/lib`
- Export `zEnv` as const object: `export const zEnv = getEnv()`
- Server crashes on invalid config (fails fast with clear Zod error messages)
...
6. **Always use zEnv** - Import `zEnv` from `lib/env.ts` instead of accessing `process.env` directly
7. **Leverage @vencura/lib** - Use `validateEnvOrThrow` or `getEnvHelper` - no custom validation logic
