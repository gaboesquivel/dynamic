...
- **Real APIs + real keys**: Use real Dynamic SDK endpoints and RPCs (testnet networks for tests) with secrets supplied via environment variables.
...
React hooks interact with live APIs via TanStack Query. Never mock query clients or requests—instantiate a fresh `QueryClient` per test suite.
...
# E2E tests (Vencura API)
bun run test:e2e
```

**Note**: All test commands use `bun` as the package manager and runtime. Vitest runs in ESM mode, testing TypeScript source files directly.
